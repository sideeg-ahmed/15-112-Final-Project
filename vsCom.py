...
def checkCollision():
	...
	if player1.hadouken and player2.hadouken and (int(player2.hit[0])  < int(player1.hit[0])):
		if (int(player2.hit[0] +player2.hit[2]+25)   > int(player1.hit[0])):
			...
			player2.boom =True
			booom = True
######################################### cooooooooommmmmmpuuuuuteeeeeeeer		
		
def danger():
	if measureSuccess(80):
		if keys[pygame.K_j] and player1.x < int(player2.body[0] + player2.body[2]) + 5:
			player2.startTimer = True
			player2.timer = 0
			return 'block'
		if  keys[pygame.K_k] and player1.x < int(player2.body[0] + player2.body[2]) + 5:
			return 'dodge'
		if keys[pygame.K_l] and not player1.lock and player1.energy >= 100:
			return 'jump'
		else:
			return False
...
def approach():
	if measureSuccess(80):
		if player2.x < player2.x + 50 < player1.x -90 and not player1.heavy:
			
			player2.fdash = True
			#player2.lock = True
		elif player2.x + int(player2.body[2])+20 < int(player1.body[0])-50:
			player2.right = True
...
def attack():
	...
	if measureSuccess(80):
		...
		if b > 8 and player2.energy == 100:
			player2.bdash = True

			player2.startHadoTimer = True
			player2.keepDistance = True
		elif keys[pygame.K_SEMICOLON] and player2.x + int(player2.body[2])+20 > int(player1.body[0])-50:
			player2.heavy = True	
		elif b <5:
			player2.punch = True
		else:
...
while not done:
	...
	if keys[pygame.K_RSHIFT] and not player1.isJump:
	   ...
	   ################################### ccccccoooooommmppuuuuuuteer
	if  not player2.isJump and not player2.lock and comCount%2==1:
		if danger() == 'block':
			player2.lock = True
			player2.blck = True
		elif player2.timer > 4:
			player2.timer = 0
			player2.startTimer = False
			player2.blck = False
		if danger() == 'dodge' :
			if player2.x > 20-player2.width/2:
				player2.bdash = True
	...
	if player2.hadoTimer == 5 :
		...
		player2.energy -= 100
		player2.hadoTimer = 0
		player2.startHadoTimer = False
		player2.keepDistance = False
		#player2.lock = True
	...
	#move Left 
	if keys[pygame.K_LEFT] and player1.x > player1.speed-player1.width/2 and not player1.lock and player1.x > int(player2.body[0]) - 10:
		if player1.isJump:
